...
void controller_work(){
    ...
    if (pthread_join(tid_gateway)){
        printf("tid_gateway join error\n");
    }
    if (pthread_join(tid_response)){
        printf("tid_response join error\n");
    }
    if (pthread_join(tid_deliver)){
        printf("tid_deliver join error\n");
    }
    if (pthread_join(tid_monitor)){
        printf("tid_monitor join error\n");
    }
    if (pthread_join(tid_monitor_dbg)){
        printf("tid_monitor join error\n");
    }

}

void controller_monitor_dbg(){
...
void service_work(){
    ...
    while(1){
        ...
        MPI_NSend(&a, 1, MPI_INT, src, 51, mcmpi_comm);
