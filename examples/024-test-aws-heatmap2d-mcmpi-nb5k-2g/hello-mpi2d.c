...
#define NXPROB      5000                 /* x dimension of problem grid */
#define NYPROB      5000                 /* y dimension of problem grid */
#define STEPS       1000                /* number of time steps */
#define MAXWORKER   48                  /* maximum number of worker tasks */
...
/* First, find out my taskid and how many tasks are running */
   ...
   printf("hello world from rank %d %s\n", taskid, processor);
   ...
   if (taskid == MASTER) {
      ...
      printf("now wait for results from all worker\n");
   ...
   if (taskid != MASTER) 
   {
      ...
      source = MASTER;
      msgtype = BEGIN;
      printf("task=%d  recv 0\n",taskid);
      MPI_Recv(&offset, 1, MPI_INT, source, msgtype, MPI_COMM_WORLD, &status);
      printf("task=%d  recv 0 ok\n",taskid);

      printf("task=%d  recv 1\n",taskid);
      MPI_Recv(&rows, 1, MPI_INT, source, msgtype, MPI_COMM_WORLD, &status);
      printf("task=%d  recv 1 ok\n",taskid);

      printf("task=%d  recv 2\n",taskid);
      MPI_Recv(&left, 1, MPI_INT, source, msgtype, MPI_COMM_WORLD, &status);
      printf("task=%d  recv 2 ok\n",taskid);


      printf("task=%d  recv 3\n",taskid);
      MPI_Recv(&right, 1, MPI_INT, source, msgtype, MPI_COMM_WORLD, &status);
      printf("task=%d  recv 3 ok\n",taskid);


      printf("task=%d  recv 4\n",taskid);
      MPI_Recv(&u[0][offset][0], rows*NYPROB, MPI_FLOAT, source, msgtype, 
               MPI_COMM_WORLD, &status);
      printf("task=%d  recv 4 ok\n",taskid);
      ...
      {
         printf("xxx Task %d it %d work.\n", taskid, it);
         if (left != NONE)
         {
             printf("xxx Task %d it %d left != NONE send to %d.\n", taskid, it, left);
            MPI_Send(&u[iz][offset][0], NYPROB, MPI_FLOAT, left,
                     RTAG, MPI_COMM_WORLD);
             printf("xxx Task %d it %d left != NONE send to %d ok.\n", taskid, it, left);
            source = left;
            msgtype = LTAG;
             printf("xxx Task %d it %d left != NONE recv from %d.\n", taskid, it, source);
            MPI_Recv(&u[iz][offset-1][0], NYPROB, MPI_FLOAT, source,
                      msgtype, MPI_COMM_WORLD, &status);
             printf("xxx Task %d it %d left != NONE recv from %d ok.\n", taskid, it, source);
         }
         if (right != NONE)
         {
             printf("xxx Task %d it %d right != NONE send to %d.\n", taskid, it, right);
            MPI_Send(&u[iz][offset+rows-1][0], NYPROB, MPI_FLOAT, right,
                      LTAG, MPI_COMM_WORLD);
             printf("xxx Task %d it %d right != NONE send to %d ok.\n", taskid, it, right);
            source = right;
            msgtype = RTAG;
             printf("xxx Task %d it %d right != NONE recv from %d.\n", taskid, it, source);
            MPI_Recv(&u[iz][offset+rows][0], NYPROB, MPI_FLOAT, source, msgtype,
                      MPI_COMM_WORLD, &status);
             printf("xxx Task %d it %d right != NONE recv from %d ok.\n", taskid, it, source);
         }
         /* Now call update to update the value of grid points */
         printf("xxx Task %d it %d to update.\n", taskid, it);
         update(start,end,NYPROB,&u[iz][0][0],&u[1-iz][0][0]);
         printf("xxx Task %d it %d to update ok.\n", taskid, it);
         iz = 1 - iz;
      }
